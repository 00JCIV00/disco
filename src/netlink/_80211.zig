...
pub const BasicServiceSet = struct {
    ...
    INFORMATION_ELEMENTS: ?ies.InformationElements = null,
    ...
    BEACON_IES: ?ies.InformationElements = null,
    ...
    pub fn getSecurityInfo(self: *const @This()) !wifi.SecurityInfo {
        var security: wifi.SecurityInfo = .{
            .type = .open,
            .auth = .open,
        };
        const bss_ies = self.INFORMATION_ELEMENTS orelse return error.MissingIEs;
        // Check RSN IE for WPA2/WPA3
        if (bss_ies.RSN) |rsn| {
            var has_psk = false;
            var has_sae = false;
            var has_eap = false;
            if (rsn.AKM_SUITES) |akm_suites| {
                for (akm_suites) |suite| {
                    // Check if RSN OUI
                    if (mem.eql(u8, suite.OUI[0..], &.{ 0x00, 0x0F, 0xAC })) {
                        const AKM = comptime c(ies.InformationElements.RobustSecurityNetwork.RSN.AKM);
                        switch (suite.TYPE) {
        ...
        if (bss_ies.VENDOR_SPECIFIC) |vendors| {
            for (vendors) |vendor| {
                if ( //
                    vendor.len >= 4 and //
...
pub fn determineAuthAlg(bss: BasicServiceSet) AUTHTYPE {
    ...
    const akmLessThan = struct {
        pub fn lessThan(_: void, a: ies.InformationElements.RobustSecurityNetwork.Suite, b: ies.InformationElements.RobustSecurityNetwork.Suite) bool {
            return a.TYPE < b.TYPE;
        }
    }.lessThan;
    const akm = sort.max(
        ies.InformationElements.RobustSecurityNetwork.Suite,
        akms,
        {},
        akmLessThan,
    ) orelse return .OPEN;
    const akm_type = meta.intToEnum(ies.InformationElements.RobustSecurityNetwork.RSN.AKM, akm.TYPE) catch {
...
pub fn requestRegisterFrames(
    ...
    const info = ctrl_info orelse return error.NL80211ControlInfoNotInitialized;
    if (types.len == 0) //
        return error.NoFramesForRegistrationProvided;
    if (matches.len != types.len) //
        return error.FramesAndMatchesLengthMismatch;
    const attrs: []const nl.Attribute = attrs: {
        var attr_list: ArrayList(nl.Attribute) = .empty;
        errdefer {
            for (attr_list.items) |attr| //
                alloc.free(attr.data);
...
pub fn requestAssociate(
    ...
    const security: wifi.SecurityType = (try bss.getSecurityInfo()).type;
    var ie_bytes: []const u8 = try nl.parse.toBytes(alloc, ies.InformationElements, bss_ies);
    ...
    switch (security) {
        .wpa2, .wpa3t, .wpa3 => {
            ...
            const new_ies: ies.InformationElements = .{
                .RSN = rsn,
                .EXTENDED_CAPABILITIES = bss_ies.EXTENDED_CAPABILITIES orelse &@as([10]u8, @splat(0)),
                .SUPPORTED_OPER_CLASSES = op_classes,
            };
            alloc.free(ie_bytes);
            ie_bytes = try nl.parse.toBytes(alloc, ies.InformationElements, new_ies);
            try attr_list.appendSlice(alloc, &.{
                .{ 
                ...
                .{ 
                    ...
                    .data = mem.toBytes(suites.CIPHER.CCMP)[0..],
                },
                .{ 
                ...
                .{
                    ...
                    .data = mem.toBytes(suites.AKM.PSK)[0..],
                },
                .{ 
...
//    const bss_ies = bss.INFORMATION_ELEMENTS orelse return error.MissingIEs;
//    const ie_bytes = try nl.parse.toBytes(alloc, ies.InformationElements, bss_ies);
//    defer alloc.free(ie_bytes);
//    Thread.sleep(config.delay * time.ns_per_ms);
//    const bssid = bss.BSSID;
...
//        const rsn = bss_ies.RSN orelse return error.MissingRSN;
//        const bytes = try nl.parse.toBytes(alloc, ies.InformationElements.RobustSecurityNetwork, rsn);
